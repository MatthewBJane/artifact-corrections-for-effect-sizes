...
For our study we want to estimate the population correlation of the unrestricted scores of the independent ($x_\mathcal{U}$) and dependent variable ($y_S$). We can denote this correlation as $\rho_{xy_S}$. The restricted population correlation can be denoted as $\rho_{xy_S}$. Within a study that suffers from range restriction, the study correlation ($r_{xy_{\mathcal{R}}}$) will be biased relative to our estimand, $\rho_{xy_{\mathcal{U}}}$. This bias can be denoted by $a$ such that,
...
It starts to become apparent that if $u_x>1$ (i.e., $\sigma_x>\sigma_\mathcal{x_S}$) the observed correlation over-estimates the true, unrestricted correlation and under-estimates the unrestricted correlation when $u_x<1$ [i.e., $\sigma_x<\sigma_\mathcal{x_S}$, @sackett2000].
